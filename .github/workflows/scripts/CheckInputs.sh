...
# Check TYPE
if [[ ! $TYPE =~ ^(regular|validator)$ ]]; then
...
# Check ENODE
if [[ ! $ENODE =~ ^enode:\/\/[0-9a-f]{128}@((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])\.){3}(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9]):30303$ ]]; then
	echo "ERROR: invalid input 'enode'"
	echo "ERROR: enode address $ENODE is incorrect. Possible causes: incorrect number of characters, invalid IP address format, specified port is different to 30303, or any other kind of malformed address."
	exit 1
fi
...
ip="$(echo $ENODE | sed -n "s/^.*@\(\S*\):30303$/\1/p")"
